...
curl -fLs --create-dirs "https://github.com/casey/just/releases/download/${VER}/just-${VER}-${OS_ARCH}-unknown-linux-musl.tar.gz" -o "/tmp/just-${VER}-${OS_ARCH}-unknown-linux-musl.tar.gz"
...
cd -
mkdir -p /tmp/just && tar -xzf "/tmp/just-${VER}-${OS_ARCH}-unknown-linux-musl.tar.gz" -C /tmp/just/
...
rm "/tmp/just-${VER}-${OS_ARCH}-unknown-linux-musl.tar.gz"
rm -r /tmp/just/
